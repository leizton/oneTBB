  ...
  public:
    ...
    segment_index_t insert_new_node(bucket* b, node_base* n, hashcode_t mask) {
        ...
        if (sz >= mask) {                                   // TODO: add custom load_factor
            ...
            if (!itt_hide_load_word(my_table[new_seg]) && as_atomic(my_table[new_seg]).compare_and_swap(is_allocating, NULL) == NULL)
...
class hash_map_iterator
  ...
  public:
    ...
    hash_map_iterator(const hash_map_iterator<Container, typename Container::value_type>& other) : my_map(other.my_map),
...
template <typename Container, typename Value>
hash_map_iterator<Container, Value>::hash_map_iterator(const Container& map, size_t index, const bucket* b, node_base* n) : my_map(&map),
                                                                                                                            my_index(index),
                                                                                                                            my_bucket(b),
...
class hash_map_range {
  ...
  public:
    ...
    //! Split range.
    hash_map_range(hash_map_range& r, split) : my_end(r.my_end),
    ...
    //! type conversion
    template <typename U>
    hash_map_range(hash_map_range<U>& r) : my_begin(r.my_begin),
                                           my_end(r.my_end),
                                           my_midpoint(r.my_midpoint),
                                           my_grainsize(r.my_grainsize) {}
    //! Init range with container and grainsize specified
    hash_map_range(const map_type& map, size_type grainsize_ = 1) : my_begin(Iterator(map, 0, map.my_embedded_segment, map.my_embedded_segment->node_list)),
    ...
    class bucket_accessor : public bucket::scoped_t {
      ...
      public:
        ...
        inline void acquire(concurrent_hash_map* base, const hashcode_t h, bool writer = false) {
            ...
            if (itt_load_word_with_acquire(my_b->node_list) == internal::rehash_req && try_acquire(my_b->mutex, /*write=*/true)) {
...
/**
 * - find()
 *   lookup(op_insert=false, key, NULL, &result, write=false,
 *          allocate_node=&do_not_allocate_node, tmp_n=nullptr)
 * 
 * - insert()
 *   lookup(op_insert=true, value.first, &value.second, &result, write=true,
 *          allocate_node=&allocate_node_copy_construct, tmp_n=nullptr)
 */
template <typename Key, typename T, typename HashCompare, typename A>
...
template <typename Key, typename T, typename HashCompare, typename A1, typename A2>
inline bool operator!=(const concurrent_hash_map<Key, T, HashCompare, A1>& a, const concurrent_hash_map<Key, T, HashCompare, A2>& b) { return !(a == b); }

template <typename Key, typename T, typename HashCompare, typename A>
inline void swap(concurrent_hash_map<Key, T, HashCompare, A>& a, concurrent_hash_map<Key, T, HashCompare, A>& b) { a.swap(b); }
